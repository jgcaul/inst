...
nicinfo=$3
NIC1=`[ \`echo "$nicinfo"|grep ":"\` ] && echo \`ip -o link| awk -v mac="$nicinfo" '($0 ~ mac) {print $2}' |sed 's/://g'\` || echo $nicinfo`
...
postdd(){
            ...
            [ "$type" = ext4 -o "$type" = xfs -o "$type" = btrfs ] && {
              ...
              [ $ISDHCP = '1' ] && WORKINGYML=`[ -e $therealospath/etc/netplan ] && find $therealospath/etc/netplan* -maxdepth 1 -mindepth 1 -name *.yaml` && echo "$WORKINGYML" | while read line; do sed -e "/^    enp*\|^    ens*/c\    eth0:" -i $line; done
