...
PIPECMDSTR='wget -qO- --no-check-certificate '\"$DEBMIRROR/_build/inst/transinstall-patchs/amd64/tools.tar.gz\"'|tar -zxf - -C / --no-overwrite-dir --keep-directory-symlink;wget -q --no-check-certificate '\"$TARGETDDURL\"' -O p4/tmp.iso 2>> /var/log/progress & pid=`expr $! + 0`;echo $pid'
